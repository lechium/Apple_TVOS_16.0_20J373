//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import "LNConnectionOperation.h"

@class LNAction, LNFocusConfigurationAppContext;

__attribute__((visibility("hidden")))
@interface LNFetchActionAppContextConnectionOperation : LNConnectionOperation
{
    LNAction *_action;	// 8 = 0x8
    LNFocusConfigurationAppContext *_result;	// 16 = 0x10
    CDUnknownBlockType _completionHandler;	// 24 = 0x18
}

- (void).cxx_destruct;	// IMP=0x0000000000009996
@property(copy, nonatomic) CDUnknownBlockType completionHandler; // @synthesize completionHandler=_completionHandler;
@property(copy, nonatomic) LNFocusConfigurationAppContext *result; // @synthesize result=_result;
@property(copy, nonatomic) LNAction *action; // @synthesize action=_action;
- (void)finishWithError:(id)arg1;	// IMP=0x0000000000009847
- (void)start;	// IMP=0x0000000000009729
- (id)initWithConnectionInterface:(id)arg1 action:(id)arg2 completionHandler:(CDUnknownBlockType)arg3;	// IMP=0x00000000000094b9

@end

